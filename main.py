...
# E'tibor bering: loyihangizda transliterate moduli to'g'ri sozlangan bo'lishi kerak
...
if not TOKEN:
    # Agarda .env ishlamasa, tokengizni to'g'ridan-to'g'ri shu yerga yozishingiz ham mumkin
    TOKEN = os.getenv("BOT_TOKEN")
...
def main_menu():
    ...
    markup.row(portfolio_button) # Alchida qatorda keng turishi uchun
    markup.add(
...
def channels_menu():
    ...
    markup.add(
        ...
        types.KeyboardButton("📋 Kanallarni ko'rish"),
...
def handle_menu(message):
    ...
    if text == "⬅️ Asosiy menyuga qaytish":
        ...
        msg = bot.send_message(chat_id, info_text, parse_mode="Markdown", reply_markup=back_markup())
        ...
        msg = bot.send_message(chat_id, "Yangi kanal nomini yoki havolasini yuboring:", reply_markup=back_markup())
...
def show_channels(message):
    ...
    if uid in channels_data and len(channels_data[uid]) > 0:
        ...
        bot.send_message(message.chat.id, "📭 Saqlangan kanallar yo'q.", reply_markup=channels_menu())
...
def choose_unit_for_add(message):
    ...
    if message.text == "🆕 Yangi Unit yaratish":
        msg = bot.send_message(message.chat.id, "✏️ Yangi Unit nomini kiriting:", reply_markup=back_markup())
...
def start_quiz(message, unit_name, index=0):
    ...
    if unit_words:
        if index < len(unit_words):
            ...
            # Barcha savollar tugaganda ushbu qism ishlaydi
            quiz_sessions[chat_id]['active'] = False
            bot.send_message(
                chat_id, 
                "🎉 Viktorina tugadi! Barcha so'zlarni takrorladingiz.", 
                reply_markup=main_menu()
            )
            return
    else:
        bot.send_message(chat_id, f"📭 **{unit_name}** bo'limi bo'sh.", parse_mode="Markdown", reply_markup=main_menu())

def countdown_worker(chat_id, msg_id, unit_name, question, answer, quiz_id):
    # API limitga tushmaslik uchun har 5 soniyada yangilanadi
...
def check_quiz_answer(message):
    ...
    if chat_id not in quiz_sessions or not quiz_sessions[chat_id].get('active'):
        msg = bot.send_message(chat_id, "➡️ Keyingi savol uchun ➡️ tugmasini bosing.", reply_markup=quiz_menu())
        bot.register_next_step_handler(msg, check_quiz_answer)
        return
